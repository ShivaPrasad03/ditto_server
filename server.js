...
app.post("/signup", async (req, res) => {
  ...
  if(bool){
  return res.status(400).send({message:"This email is already registered with Ditto."})
  }
  const new_data = new User({ username, email, password });
  new_data.save();
  return res.status(200).send({message:"Your Ditto Account Created Successfully!!"});
});
